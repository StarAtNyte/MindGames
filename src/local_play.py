...
USE_4BIT = True  # 4-bit quantization (recommended for RTX 4090)
team_hash = "MG25-3162A7F500"

# Create logs directory
LOGS_DIR = "game_logs_local"
os.makedirs(LOGS_DIR, exist_ok=True)

def print_banner():
    print("\n" + "=" * 70)
    print("SecretMafia Agent - LOCAL INFERENCE MODE")
...
agent = LocalStreamlinedMafiaAgent(
        ...
        try:
            for role, stats in role_stats.items():
                if stats and isinstance(stats, dict) and stats.get('games', 0) > 0:
                    ...
                    emoji = role_emojis.get(str(role).lower(), '❓')
...
# Try importing textarena
try:
    import textarena as ta

    print(f"\n📡 Connecting to online server (using local LLM)...")
    print(f"   Team: {team_hash}")
    ...
    print(f"╠{'═'*60}╣")
    print(f"║ ✅ Completed {game_count:>2d} Local LLM Games{' '*28}║")
    ...
    print("❌ TextArena not installed. Install with: pip install textarena")
    print("Running in test mode instead...\n")

    # Test mode - similar to old test_local_agent
    print("="*70)
    print("TESTING LOCAL AGENT WITH SAMPLE OBSERVATIONS")
    print("="*70)

    test_cases = [
        {
            "name": "Discussion Phase",
            "observation": """Welcome to Secret Mafia!
You are Player 1.
Your role: Villager
...
Player 3: We need to find the Mafia!"""
        },
        {
            "name": "Voting Phase",
            "observation": """Day breaks. Discuss for 3 rounds.
Player 0: I think Player 3 is suspicious.
Player 2: I agree with Player 0.
Player 3: I'm innocent!

Voting phase - submit one vote in format [X]. Valid: [0], [2], [3]"""
        },
        {
            "name": "Night Phase (Villager)",
            "observation": """Player 2 was eliminated by vote.

Night has fallen. Villagers sleep.
Wait for the next day to begin."""
    ...
    for result in results:
        ...
        print(f"{result['test']:<30} {status} {time_str}")
