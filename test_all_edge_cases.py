...
class EdgeCaseTester:
    """Test all possible edge cases and failure modes"""
    ...
    def test_invalid_observations(self) -> bool:
        ...
        for name, obs in test_cases:
            print(f"\n   Testing: {name}")
            try:
                agent = StreamlinedMafiaAgent(modal_endpoint_url=self.endpoint_url)
                agent.my_role = "villager"
                agent.my_player_id = 1
                agent.alive_players = [0, 1, 2, 3]
                agent.turn_count = 2
    ...
    def test_consecutive_games_stability(self) -> bool:
        ...
        for game_num in range(num_games):
            ...
            try:
                ...
                agent.my_role = ["mafia", "detective", "doctor", "villager"][game_num % 4]
                agent.my_player_id = game_num % 4
                agent.alive_players = [0, 1, 2, 3]
                agent.turn_count = game_num
...
Player 3: Let's vote carefully."""
    ...
    def test_response_format_validation(self) -> bool:
        ...
        for i, case in enumerate(test_cases):
            ...
            try:
                agent = StreamlinedMafiaAgent(modal_endpoint_url=self.endpoint_url)
                agent.my_role = "detective" if "Detective" in case['name'] else "villager"
                agent.my_player_id = 1
                agent.alive_players = [0, 1, 2, 3]
                agent.turn_count = 2

                response = agent(case['obs'])
    ...
    def test_player_count_variations(self) -> bool:
        ...
        for count in player_counts:
            ...
            try:
                ...
                agent.my_role = "villager"
                agent.my_player_id = 0
                agent.alive_players = list(range(count))
                agent.turn_count = 1
    ...
    def test_memory_overflow_protection(self) -> bool:
        ...
        try:
            ...
            for turn in range(30):
                agent.turn_count = turn
                agent.memory.investigation_results[turn % 4] = "MAFIA" if turn % 2 == 0 else "INNOCENT"
                agent.memory.discussion_history.append(f"Turn {turn} discussion" * 10)

            # Check memory sizes
            investigation_count = len(agent.memory.investigation_results)
            discussion_count = len(agent.memory.discussion_history)
...
Player 2: Not sure yet."""
    ...
    def print_summary(self):
        ...
        if passed == total:
            ...
            print("Agent is deployable with known limitations")
            return True
        else:
...
def main():
    ...
    tester = EdgeCaseTester(endpoint_url)
